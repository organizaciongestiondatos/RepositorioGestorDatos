sap.ui.define([
...
], function (ODataModel, Controller, MessageToast, Message, MessageType) {
	"use strict";
	return Controller.extend("SAPEspania.SAPEspania.controller.Main", {
		...
		buscar: function () {
			...
			this.compruebaFormatoDNI(oView.byId("input00").getValue() );
		...
		compruebaFormatoDNI : function (dni) {
		//mleme mlem lem
			var numero;
			var letr;
			var letra;
			...
			if (expresion_regular_dni.test(dni) == true) {
				...
				if (letra != letr.toUpperCase()) {
					MessageToast.show('DNI erroneo, la letra del NIF no se corresponde');
				} else {
					MessageToast.show('DNI correcto');
				}
			} else {
				
				
				MessageToast.show('DNI erroneo, formato no válido');
			}
		},
